...
class NeuralFluid(object):
    def __init__(self, cfg):
        ...
        self.velocity_field = get_network(cfg, 3, 2).cuda()
        self.pressure_field = get_network(cfg, 3, 1).cuda()
        # self.velocity_field = self._gt_taylorgreen
        # self.pressure_field = self._gt_taylorgreen_p
        self.device = torch.device("cuda:0")
    
    def _gt_taylorgreen(self, samples, time):
        func = get_source_velocity("taylorgreen")
        return func(samples) + 0 * time

    def _gt_taylorgreen_p(self, samples, time):
        x = (samples[..., 0] + 1) * torch.pi
        y = (samples[..., 1] + 1) * torch.pi
        p = 0.25 * (torch.cos(2 * x) + torch.cos(2 * y)) / torch.pi / torch.pi
        return p
    ...
    def _train_step(self, source_func):
        ...
        u_init = self.velocity_field(x_init, t_init)
        ...
        vel_x = self.velocity_field(bc_sample_x, t_bound)[..., 0]
        vel_y = self.velocity_field(bc_sample_y, t_bound)[..., 1]
        ...
        u_main = self.velocity_field(x_main, t_main)
        p_main = self.pressure_field(x_main, t_main)
    ...
    def sample_velocity_field(self, resolution, t, to_numpy=True, with_boundary=False, return_samples=False, require_grad=False):
        ...
        out = self.velocity_field(grid_samples, time)
        if to_numpy:
    ...
    def draw_velocity(self, resolution, t):
        grid_values, grid_samples = self.sample_velocity_field(resolution, t, to_numpy=True, with_boundary=True, return_samples=True)
