...
struct Image {
	...
	Image(int h, int w, float val): buffer(h * w, Array<DIM>(val)), h(h), w(w) {};

	Image(const std::vector<std::vector<float>> mat) {
		h = mat.size();
		w = mat[0].size();
	...
	Array<DIM> get(Vector2 uv, bool flipY = false) const {
